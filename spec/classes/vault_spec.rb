...
describe 'vault' do
  on_supported_os.each do |os, facts|
    ...
    context 'x86' do
      let(:facts) { facts.merge({:architecture => 'x86'}) }
      it {
        is_expected.to contain_staging__deploy('vault.zip')
                           .with_source('https://releases.hashicorp.com/vault/5.0.1/vault_5.0.1_linux_x86.zip')
      }
    end

    context 'arch => x86' do
      let(:facts) { facts.merge({:architecture => 'amd64'}) }
      let(:params) { {:arch => 'x86'} }
      it {
        is_expected.to contain_staging__deploy('vault.zip')
                           .with_source('https://releases.hashicorp.com/vault/5.0.1/vault_5.0.1_linux_x86.zip')
    ...
    context 'architecture => x86_64' do
      let(:facts) { facts.merge({:architecture => 'x86_64'}) }
      it {
        is_expected.to contain_staging__deploy('vault.zip')
                           .with_source('https://releases.hashicorp.com/vault/5.0.1/vault_5.0.1_linux_amd64.zip')
      }
    end
